...
class Strong_complete(bt.Strategy): # bt.Strategy를 상속한 class로 생성해야 함.
    ...
    def next(self):
        ...
        for i in range(len(self.datas)//3):
            data_idx = i * 3
            need_continous = 4 # 연속 유지 조건일 수보다 -1
            for day in range(-need_continous,1):
                if self.ind_dict[str(i)+'_sto_weekly_20'].percK[day] > self.ind_dict[str(i)+'_sto_weekly_20'].percK[day-1]:
                    cond_A = True
                else:
                    cond_A = False
                    break
            ...
            for day in range(-need_continous,1):
                if self.ind_dict[str(i)+'_sto_weekly_20'].percK[day] >= self.ind_dict[str(i)+'_sto_weekly_20'].percD[day]:
                    cond_B = True
                else:
                    cond_B = False
                    break
            ...
            for day in range(-need_continous,1):
                if self.ind_dict[str(i)+'_sto_weekly_18'].percK[day] > self.ind_dict[str(i)+'_sto_weekly_18'].percK[day-1]:
                    cond_L = True
                else:
                    cond_L = False
                    break
            ...
            for day in range(-need_continous,1):
                if self.ind_dict[str(i)+'_sto_weekly_18'].percK[day] >= self.ind_dict[str(i)+'_sto_weekly_18'].percD[day]:
                    cond_N = True
                else:
                    cond_N = False
                    break
            ...
            if cond_A and cond_B and cond_C and cond_D and \
                    ...
                    cond_I and cond_J and cond_K and cond_L and \
                    cond_M and cond_N:
                # print('종목 번호',i)
...
# 매도 전략
class MR_530(bt.Strategy): # bt.Strategy를 상속한 class로 생성해야 함.
    ...
    def __init__(self):
        ...
        for i in range(len(self.datas)): # 각 종목별 지표 계산
            ...
            if i % 3 == 0:# daily data
                ...
                self.ind_dict[num+'_sma_daily_224'] = bt.ind.SMA(self.datas[i], period=224)
                self.ind_dict[num+'_sma_daily_20'] = bt.ind.SMA(self.datas[i], period=20)
                self.ind_dict[num+'_sma_daily_10'] = bt.ind.SMA(self.datas[i], period=10)
                self.ind_dict[num+'_sto_daily_18'] = Stochastic_slow_daeshin(self.datas[i],
                                                                              period=18,
                                                                              period_dfast=10,
                                                                              period_dslow=10)
                self.ind_dict[num+'_sto_daily_5'] = Stochastic_slow_daeshin(self.datas[i],
    ...
    def next(self):
        ...
        for i in range(len(self.datas)//3):
            ...
            need_continous = 4 # 연속 유지 조건일 수보다 -1
            for day in range(-need_continous,1):
                if self.ind_dict[str(i)+'_sto_daily_18'].percK[day] > self.ind_dict[str(i)+'_sto_daily_18'].percK[day-1]:
                    cond_F = True
                else:
                    cond_F = False
                    break
            ...
            for day in range(-need_continous,1):
                if self.ind_dict[str(i)+'_sto_daily_18'].percD[day] > self.ind_dict[str(i)+'_sto_daily_18'].percD[day-1]:
                    cond_G = True
                else:
                    cond_G = False
                break

            cond_K = self.ind_dict[str(i)+'_sto_daily_5'].percK[0] < 45
            cond_L = self.datas[data_idx].close[0] >= self.ind_dict[str(i)+'_sma_daily_20'][0]
